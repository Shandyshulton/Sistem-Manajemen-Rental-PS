...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('bookings.update', $booking->id_booking) }}" method="POST" id="bookingForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Phone Number <span class="text-danger">*</span></label><p>
                            <input type="tel" name="phone_number" id="phone_number" class="form-control" value="{{ old('phone_number', $booking->phone_number) }}" required >
                    </div>
                </div>
                ...
                <div class="row mb-3">
                        ...
                        <select name="console_id" class="form-select" id="consoleSelect" required>
                            ...
                            @foreach ($consoles as $console)
                                <option value="{{ $console->id_console }}" data-price="{{ $console->price }}"
                                    {{ old('console_id', $booking->console_id) == $console->id_console ? 'selected' : '' }}>
                    ...
                    @php
                        $formattedStartTime = \Carbon\Carbon::parse($booking->start_time)->format('H:i');
                    @endphp
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="start_time" class="form-select" required>
                            @foreach ($timeSlots as $time)
                                <option value="{{ $time }}"
                                    {{ old('start_time', $formattedStartTime) == $time ? 'selected' : '' }}>
                                    {{ $time }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <select name="estimated_hours" class="form-select" id="durationSelect" required>
                            @for ($i = 1; $i <= 8; $i++)
                                <option value="{{ $i }}"
                                    {{ old('estimated_hours', $booking->estimated_hours) == $i ? 'selected' : '' }}>
                                    {{ $i }} hour
                                </option>
                            @endfor
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="payment_type" class="form-select" required>
                            <option value="Cash"
                                {{ old('payment_type', $booking->payment_type) == 'Cash' ? 'selected' : '' }}>Cash
                            </option>
                            <option value="Transfer"
                                {{ old('payment_type', $booking->payment_type) == 'Transfer' ? 'selected' : '' }}>
                                Transfer
                            </option>
                            <option value="QRIS"
                                {{ old('payment_type', $booking->payment_type) == 'QRIS' ? 'selected' : '' }}>QRIS
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" id="totalPayment" class="form-control"
                ...
                <div class="mb-3">
                    ...
                    <select name="selected_games[]" class="form-select" multiple required>
                        @foreach (['FIFA 23', 'Call of Duty', 'NBA 2K23', 'Mortal Kombat', 'Forza Horizon'] as $game)
                            <option value="{{ $game }}"
                                {{ in_array($game, old('selected_games', json_decode($booking->selected_games))) ? 'selected' : '' }}>
                                {{ $game }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select" required>
                        <option value="pending" {{ old('status', $booking->status) == 'pending' ? 'selected' : '' }}>
                            Pending</option>
                        <option value="success" {{ old('status', $booking->status) == 'success' ? 'selected' : '' }}>
                            Success</option>
                ...
                <div class="mb-3">
                    <label class="form-label">Playing Status</label>
                    <select name="playing_status" class="form-select" required>
                        <option value="Play"
                            {{ old('playing_status', $booking->playing_status) == 'Play' ? 'selected' : '' }}>Play
                        </option>
                        <option value="Not Play"
                            {{ old('playing_status', $booking->playing_status) == 'Not Play' ? 'selected' : '' }}>Not
                            Play</option>
                    </select>
                </div>
